...
// Used for secure seed phrase generation
...
const BIP39_WORDLIST = [
  ...
  'hazard', 'head', 'health', 'heart', 'heavy', 'hedgehog', 'height', 'held', 'help', 'hen',
...
// Secure seed phrase generator using cryptographically secure random
function generateSecureSeedPhrase(wordCount = 12) {
...
function validateSeedPhrase(seedPhrase) {
  if (!seedPhrase || typeof seedPhrase !== 'string') {
    return { valid: false, error: 'Seed phrase must be a string' };
  }
  
  const words = seedPhrase.trim().split(/\s+/);
  
  if (![12, 15, 18, 21, 24].includes(words.length)) {
    return { valid: false, error: 'Seed phrase must contain 12, 15, 18, 21, or 24 words' };
  }
  
  for (let i = 0; i < words.length; i++) {
    if (!BIP39_WORDLIST.includes(words[i].toLowerCase())) {
      return { valid: false, error: `Word "${words[i]}" at position ${i + 1} is not in BIP39 wordlist` };
    }
  }
...
window.generateSecureSeedPhrase = generateSecureSeedPhrase;
window.validateSeedPhrase = validateSeedPhrase;