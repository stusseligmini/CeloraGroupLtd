...
(function () {
  ...
  class FirebaseAuthClient {
    ...
    async signInWithEmailAndPassword(email, password) {
      try {
        const response = await fetch(`${API_BASE}:signInWithPassword?key=${FIREBASE_CONFIG.apiKey}`, {
          method: 'POST',
    ...
    async createUserWithEmailAndPassword(email, password) {
      try {
        const response = await fetch(`${API_BASE}:signUp?key=${FIREBASE_CONFIG.apiKey}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            email: email,
