...
describe('registerUsername', () => {
  ...
  test('success registers', async () => {
    mockFetch(async () => new Response(JSON.stringify({ username: 'ok', address: 'A' }), { status: 200 }));
    const r = await registerUsername('ok', 'A');
    expect(r.success).toBe(true);
  });
  test('conflict returns Taken', async () => {
    mockFetch(async () => new Response(JSON.stringify({ error: 'taken' }), { status: 409 }));
