...
export function deriveAddress(
  ...
  const w = deriveWallet(mnemonic, blockchain, accountIndex, addressIndex);
...
export function deriveMultiple(
  ...
  return blockchains.map(bc => deriveAddress(mnemonic, bc, accountIndex, 0));
}
...
export async function getPublicAddressFromEncrypted(
  ...
  return deriveAddress(mnemonic, blockchain, 0, 0);
}
