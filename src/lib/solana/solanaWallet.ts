...
export function deriveSolanaWallet(mnemonic: string, accountIndex: number = 0): SolanaWallet {
  ...
  if (useLegacy) {
    ...
    const legacy = deriveWallet(mnemonic, 'solana', accountIndex);
...
/**
 * Get Solana connection with Helius RPC and WebSocket support
 */
export function getSolanaConnection(rpcUrl?: string, wsUrl?: string): Connection {
  // Use Helius devnet for testing (supports WebSocket)
