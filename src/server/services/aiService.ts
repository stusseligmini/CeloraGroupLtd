...
export class AIService {
  ...
  async analyzeFraud(
    ...
    if (amount > userPattern.averageAmount * 10) {
      reasons.push(`Transaction amount (${amount}) is 10x larger than average (${userPattern.averageAmount.toFixed(2)})`);
      riskScore += 30;
    }

    // Rule 2: First-time recipient with large amount
    const isNewRecipient = !userPattern.commonRecipients.includes(recipient);
    if (isNewRecipient && amount > userPattern.averageAmount * 3) {
    ...
    if (currentHour < 6 || currentHour > 22) {
      if (userPattern.commonTime !== 'night') {
